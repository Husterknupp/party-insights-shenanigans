...
function load(html, maybeOptions, isDocument) {
  var loadedCheerio = Cheerio.load(html, maybeOptions, isDocument);
  var applyElementToCheerioUnsafe = (function(element, selectorFunction) {
        return selectorFunction(element)
      });
  return function (unsafeFromJsLand, basicAcceptedElems) {
    if (unsafeFromJsLand !== undefined) {
      return applyElementToCheerioUnsafe(Caml_option.valFromOption(unsafeFromJsLand), loadedCheerio);
    } else {
...
function getColspanInt(element) {
  var parsed = Core__Int.fromString(getColspan(element), undefined);
  if (parsed !== undefined) {
    return parsed;
  } else {
    return 1;
...
function getRowspan(element) {
  ...
  var rowspan = Core__Nullable.getExn(element.attribs).rowspan;
  if (rowspan == null) {
    return "1";
  } else {
    return rowspan;
...
function getRowspanInt(element) {
  var parsed = Core__Int.fromString(getRowspan(element), undefined);
  if (parsed !== undefined) {
    return parsed;
  } else {
    return 1;
...
function getSrc(element) {
  return Caml_option.nullable_to_opt(element.attr("src"));
}
